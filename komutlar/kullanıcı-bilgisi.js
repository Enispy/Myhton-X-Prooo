...
//BU KOMUT CODARE SUNUCUSUNDAN YUSUF A AİTTİR!
      ...
      //BU KOMUT CODARE SUNUCUSUNDAN YUSUF A AİTTİR!
        ...
        .addField(`Durumu`, userinfo.status, false)
        .setColor('ORANGE')
        .addField(`Katılım Tarihi (Sunucu)`, userinfo.dctarihkatilma, false)
        .addField(`Katılım Tarihi (Discord)`, userinfo.dctarih, false)
        .addField(`İD:`, userinfo.id, true)
        .addField(`Bot mu?:`, userinfo.bot, true)
        .addField(`Rolleri:`, `${msg.guild.members.get(user.id).roles.filter(r => r.name !== "@everyone").map(r => r).join(' **|** ') || "**Bu kullanıcıda hiçbir rol bulunmuyor**"}`, false)
